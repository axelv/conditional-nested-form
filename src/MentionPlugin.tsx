...
function MentionsTypeaheadMenuItem({
    ...
    let className = 'item whitespace-nowrap cursor-pointer';
    if (isSelected) {
...
type MentionPluginProps<TFieldValues extends FieldValues> = {
...
export default function MentionPlugin<TFieldValues extends FieldValues>({ control }: MentionPluginProps<TFieldValues>): JSX.Element | null {
    ...
    useEffect(() => {
        const removeMutationListener = editor.registerMutationListener(MentionNode,
            (mutations) => {
                    ...
                    switch (mutation) {
                        case "created":
                            editor.getEditorState().read(() => {
                                ...
                                console.debug("created", nodeKey, node)
                                if (node instanceof MentionNode) {
                                    console.debug("Register a hidden input for the mention")
                                    setMentions((mentions) => [...mentions, { ...node.exportJSON(), key: node.getKey() }])
                                }
                            })
                            break;
                        case "destroyed": {
                            editor.getEditorState().read(() => {
                                ...
                                console.debug("Unregister the hidden input for the mention")
                                setMentions((mentions) => mentions.filter((mention) => mention.key !== nodeKey))
                            })
                            break;
                        }
                    }
    ...
    const onSelectOption = useCallback(
        (
            ...
            editor.update(() => {
                const id = selectedOption.name.toLowerCase().replace(/\s/g, "-");
                const href = `#${id}`;

                const mentionNode = $createMentionNode(selectedOption.name, href);
                if (nodeToReplace) {
        ...
        <>
            <fieldset className='sr-only'>
                ...
                {control && mentions.map((node) => <Controller
                    ...
                    render={({ field }) => <input id={`mention-${node.key}`} type='hidden' {...field} value={field.value.mention} />}
                />)}
            </fieldset>
                ...
                menuRenderFn={(
                    ...
                    anchorElementRef.current && results.length
                        ? ReactDOM.createPortal(
                            <div className="typeahead-popover mentions-menu relative top-5 bg-white rounded border border-gray-200 w-fit p-1">
                                <ul className='w-fit'>
